...
/*
    ...
    pfListOpts := portforward.ListOpts{
        Page:      1,
        Size: 	   20,    
    }
    pfPager := portforward.List(vmHandler.NetworkClient, pfListOpts) // Caution!!) Not VMClient but NetworkClient
    // Iterate through all pages and print rule info
    err := pfPager.EachPage(func(page pagination.Page) (bool, error) {
        pfList, err := portforward.ExtractPFs(page)
        if err != nil {
            return false, err
        }
		fmt.Printf("\n")
...
*/
	...
	vpcIID := irs.IID{SystemId: "60e5d9da-55cd-47be-a0d9-6cf67c54f15c"}

	var sgSystemIDs []string
	sgSystemIDs = append(sgSystemIDs, "ktcloudvp-crt5ndcvtts41jm39tcg")
	privateIP := "10.0.1.138"
	publicIPId := "9804e178-bb3d-48b5-a986-e03a44bdf7d2"

	// Create PortForwarding and Firewall Rules
	if ok, err := vmHandler.createPortForwardingRules(vpcIID, sgSystemIDs, privateIP, publicIPId); !ok {
		newErr := fmt.Errorf("Failed to Create PortForwarding Rules : [%v]", err)
		cblogger.Error(newErr.Error())
		return nil, newErr
	}
*/
...
/*
    ...
    targetIP := "211.34.246.113" // public IP or private IP
    policyIDs := vmHandler.FindFirewallPolicyIDsByIP(fwRuleList, targetIP)
    fmt.Printf("PolicyIDs for IP %s: %v\n", targetIP, policyIDs)
*/
...
/*
	cblogger.Info("Cloud driver: called Create()!!")
	 // Define the firewall rule creation options
    createOpts := rules.CreateOpts{


		// SrcNat:           	false,
		// SrcInterface:	  	externalVLanName,
		// Action:           	true, // accept
		// Protocol:      		curProtocol,
		// StartPort:     		sgRule.FromPort,
		// EndPort:       		sgRule.ToPort,
		// DstNetwork: 		[]string{tierNetworkId},
		// SrcAddress:       	[]string{srcIPAdds},
		// DstAddress:       	[]string{destCIDR}, // Cannot be entered simultaneously with portForwardingId
		// // Comment:          	comment,
        ...
        // SrcNat:           false,
		// PortForwardingId: "e28da32d-467a-4727-ac12-ead443c912de",
		// SrcInterface:	  "3102",
		// Action:           true,
        ...
        // // DstAddress:       []string{"10.0.1.138/32"}, // Cannot be entered simultaneously with portForwardingId
        // Comment:          "Allow HTTP traffic to 10.0.1.0/32",
    ...
    result := rules.Create(vmHandler.NetworkClient, createOpts) // Caution!!) Not VMClient but NetworkClient
    if result.Err != nil {
		newErr := fmt.Errorf("Failed to create firewall rule: %v", result.Err)
		cblogger.Error(newErr.Error())
		return nil, newErr
    } else {
		...
		if err != nil {
			newErr := fmt.Errorf("Failed to extract created rule: %v", err)
			cblogger.Error(newErr.Error())
			return nil, newErr
		}
		fmt.Printf("Created Firewall Rule: ID=%s, Protocol=%s, Action=%v\n", rule.PolicyID, rule.DstAddress, rule.Action)


		jobId, err := rules.ExtractJobID(result)
		if err != nil {
			fmt.Printf("Failed to extract jobId: %v\n", err)
		} else {
			fmt.Printf("Created jobId: %s\n", jobId)
		}
	}
...
/*
	...
	cblogger.Info("Cloud driver: called Delete(()!!")
	// The ID of the firewall rule to delete
    ruleID := "4694eb0b-dc68-4b0c-ada9-892451763b14"

    // Call Delete to remove the rule
    delResult := rules.Delete(vmHandler.NetworkClient, ruleID) // Caution!!) Not VMClient but NetworkClient

    // Check for errors
    if delResult.Err != nil {
		newErr := fmt.Errorf("Failed to delete firewall rule: %v", delResult.Err)
		cblogger.Error(newErr.Error())
		return nil, newErr
    } else {
