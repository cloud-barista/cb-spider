...
func (nlbHandler *KTVpcNLBHandler) createVMGroup(nlbId string, nlbReqInfo irs.NLBInfo) (error) {
	...
	if len(*nlbReqInfo.VMGroup.VMs) < 1 {
...
// Returns the first found subnet ID as all VMs in a VMGroup should be in the same subnet
func (nlbHandler *KTVpcNLBHandler) getSubnetOfVMGroup(ctx context.Context, vmGroup irs.VMGroupInfo) (string, error) {
    ...
    // Validate VMGroup has VMs
    if vmGroup.VMs == nil || len(*vmGroup.VMs) == 0 {
        newErr := fmt.Errorf("VMGroup has no VMs to determine subnet")
        cblogger.Error(newErr.Error())
        loggingError(callLogInfo, newErr)
        return "", newErr
    }
    ...
    // Get the first VM to determine the subnet
    vmIID := (*vmGroup.VMs)[0]
    ...
    if getErr != nil {
        newErr := fmt.Errorf("Failed to Get KT VM info: %w", getErr)
        cblogger.Error(newErr.Error())
        loggingError(callLogInfo, newErr)
        return "", newErr
    }
    ...
    // cblogger.Infof("Found subnet [%s] for VMGroup", *tierId)
    return *tierId, nil
}
