...
Loop:
	for {
		commandStr := readLine("")
		commandNum := 0
		fmt.Sscan(commandStr, &commandNum)
		...
		case 1:
			...
			resTypeStr := readLine("=== Enter resource type ===")
			...
			resName := readLine("=== Enter name of the resource ===")
			tagKey := readLine("=== Enter tag's key ===")
			tagValue := readLine("=== Enter tag's value ===")
...
func testFileSystemHandler(config Config) {
	...
	fileNameId := irs.IID{
		NameId: config.Openstack.Resources.File.IID.NameId,
	}

	vpcIID := irs.IID{
		NameId: config.Openstack.Resources.File.VpcIID.NameId,
	}

	subnetIID := irs.IID{
		NameId: config.Openstack.Resources.File.SubnetIID.NameId,
	}
...
func showTestHandlerInfo() {
	...
	cblogger.Info("12. FileSystemHandler")
