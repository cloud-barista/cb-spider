...
func acceleratorsToGPUInfoList(accerators []*compute.MachineTypeAccelerators) []irs.GpuInfo {
	...
	for _, accelerator := range accerators {
		gpuInfo := irs.GpuInfo{}
		gpuInfo.Model = strings.ToUpper(accelerator.GuestAcceleratorType)
		...
		if len(accrType) >= 3 {
			...
			if strings.HasSuffix(lastElement, "gb") {
				...
				gpuInfo.MemSizeGB = "-1" // Mem은 빈 문자열로 설정
				if len(accrType) > 1 {
		...
		if gpuCount == 0 { // 값이 0이라면 기본값 -1
			gpuCount = -1
		}
		gpuInfo.Count = strconv.FormatInt(gpuCount, 10)
