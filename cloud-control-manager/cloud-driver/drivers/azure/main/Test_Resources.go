	...
	for {
		...
		if inputCnt == 1 {
			...
			case 1:
				...
				if listRegionZone, err := priceInfoHandler.ListProductFamily(region); err != nil {
					cblogger.Error(err)
				} else {
					spew.Dump(listRegionZone)
				}
			...
			case 2:
				...
				if listRegionZone, err := priceInfoHandler.GetPriceInfo(productFamiliy, region, filterList); err != nil {
					cblogger.Error(err)
				} else {
					spew.Dump(listRegionZone)
				}
