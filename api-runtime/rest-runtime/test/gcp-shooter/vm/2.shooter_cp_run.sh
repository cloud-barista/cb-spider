...
do
	if [ $num -eq -1 ] ; then
		...
		do
			...
			scp -i ../keypair/$KEY_NAME.key -o "StrictHostKeyChecking no" ./shooter/shooter.sh powerkim-prj@$PUBLIC_IP:/tmp
			ssh -i ../keypair/$KEY_NAME.key -o "StrictHostKeyChecking no" powerkim-prj@$PUBLIC_IP /tmp/shooter.sh &
		done
