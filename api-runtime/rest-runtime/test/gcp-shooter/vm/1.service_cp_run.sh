...
echo ========================== $CONNECT_NAME
PUBLIC_IPS=`curl -sX GET http://$RESTSERVER:1024/spider/publicip/publicipt${num}-powerkim?connection_name=$CONNECT_NAME |json_pp |grep "\"PublicIP\"" |awk '{print $3}' |sed 's/"//g' |sed 's/,//g'`
# 137.135.167.9
