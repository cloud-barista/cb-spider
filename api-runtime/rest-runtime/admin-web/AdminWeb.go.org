...
func makeHeaderTRList_html(bgcolor string, fontSize string, nameWidthMap map[string]string) string {
	...
	for key := range nameWidthMap {
		str := fmt.Sprintf(`
			    <td width="%s">
				    <font size=2>%s</font>
			    </td>a
			`, key, nameWidthMap[key])
		strTR += str
	}
...
func driver(c echo.Context) error {
	...
	// (1) get driver info list @todo if empty list
...
func makeHeaderTRList_html(bgcolor string, fontSize string, nameWidthMap map[string]string) string {
	...
	// (4) make info TR list
	strData := makeDriverTRList_html("", "", "", info.ResultList)
