...
func makeMgmtTRList_html(bgcolor string, height string, fontSize string, infoList cr.AllResourceList) string {
	...
	count := 0
	for _, one := range infoList.AllList.MappedList {
		count++
		str := strings.ReplaceAll(strTR, "$$NUM$$", strconv.Itoa(count))
	...
	for _, one := range infoList.AllList.OnlySpiderList {
		count++
		str := strings.ReplaceAll(strTR, "$$NUM$$", strconv.Itoa(count))
		str = strings.ReplaceAll(str, "$$NAMEID$$", one.NameId)
		str = strings.ReplaceAll(str, "$$SYTEMID$$", "( "+one.SystemId+" )")
	...
	for _, one := range infoList.AllList.OnlyCSPList {
		count++
		str := strings.ReplaceAll(strTR, "$$NUM$$", strconv.Itoa(count))
		str = strings.ReplaceAll(str, "$$NAMEID$$", "( "+one.NameId+" )")
		str = strings.ReplaceAll(str, "$$SYTEMID$$", one.SystemId)
