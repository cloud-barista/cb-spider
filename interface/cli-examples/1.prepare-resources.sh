...
time $CLIPATH/spiderctl --config $CLIPATH/grpc_conf.yaml vpc create -i json -d \
    '{
...
time $CLIPATH/spiderctl --config $CLIPATH/grpc_conf.yaml security create -i json -d \
    '{
      "ConnectionName":"'${CONN_CONFIG}'",
      "ReqInfo": {
        "Name": "'${SG_NAME}'",
...
time $CLIPATH/spiderctl --config $CLIPATH/grpc_conf.yaml keypair create -i json -d \
    '{
      "ConnectionName":"'${CONN_CONFIG}'",
      "ReqInfo": {
        "Name": "'${KEYPAIR_NAME}'"
