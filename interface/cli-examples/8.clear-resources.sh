...
time $CLIPATH/spctl --config $CLIPATH/spctl.conf keypair delete --cname "${CONN_CONFIG}" -n "${KEYPAIR_NAME}" 2> /dev/null
echo "============== after delete KeyPair: '${KEYPAIR_NAME}'"

echo -e "\n\n"

echo "============== before delete SecurityGroup: '${SG_NAME}'"
time $CLIPATH/spctl --config $CLIPATH/spctl.conf security delete --cname "${CONN_CONFIG}" -n "${SG_NAME}" 2> /dev/null
echo "============== after delete SecurityGroup: '${SG_NAME}'"

echo -e "\n\n"

echo "============== before delete VPC/Subnet: '${VPC_NAME}'"
time $CLIPATH/spctl --config $CLIPATH/spctl.conf vpc delete --cname "${CONN_CONFIG}" -n "${VPC_NAME}" 2> /dev/null
