...
# CB-Spider VM AssignPublicIP=true Test - VM Delete Runner for All CSPs
# Terminates the 'cb-spider-truepublicip-test' VM on all 10 CSPs in parallel
# and waits for removal. Reuses ../common-vm-delete.sh (the same delete logic
# as the parent vm-default-publicip-test suite).
...
export RESULT_DIR="/tmp/vm_truepublicip_delete_results_$$"
LOG_DIR="/tmp/vm_truepublicip_delete_logs_$$"
...
echo ""
echo "################################################################################"
echo "#   CB-Spider VM AssignPublicIP=true Test - VM Delete - All CSPs              #"
echo "################################################################################"
echo ""
...
for csp in ${CSP_ORDER}; do
    ...
    (
        ...
        "${PARENT_DIR}/common-vm-delete.sh"
    ) > "${log_file}" 2>&1 &
...
echo "============================================================"
echo "          VM DELETE SUMMARY - ALL CSPs (AssignPublicIP=true)"
echo "============================================================"
echo ""
