...
# Alibaba VM Default-PublicIP Test Script
...
# IMAGE_NAME_PREFIX via GET /spider/vmimage (same approach as spiderwatch's
# resolveImageName()).
...
export RESULT_FILE="${RESULT_DIR:-/tmp/vm_publicip_results}/result_alibaba.txt"
...
if [[ -z "${resolved_image}" ]]; then
    ...
    echo "${CSP_NAME}|IMAGE_RESOLVE_ERROR|-|-|-|-|-" > "${RESULT_FILE}"
    exit 1
fi
