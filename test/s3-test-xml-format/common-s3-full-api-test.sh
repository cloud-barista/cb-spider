...
bucket_exists() {
    local code
    code=$(curl -u $API_USERNAME:$API_PASSWORD -s -o /dev/null -w '%{http_code}' -I "$SPIDER_URL/$TEST_BUCKET" \
...
cleanup_multipart_uploads() {
    ...
    uploads_response=$(curl -u $API_USERNAME:$API_PASSWORD -s -X GET "$SPIDER_URL/$TEST_BUCKET?uploads" \
    ...
    if [[ -n "$uploads_response" ]] && [[ "$uploads_response" =~ \<UploadId\> ]]; then
        ...
        echo "$uploads_response" | sed 's/<\/Upload>/\n<\/Upload>\n/g' | grep '<Upload>' | while read -r upload_block; do
            ...
            if [[ -n "$key" ]] && [[ -n "$upload_id" ]]; then
                ...
                curl -u $API_USERNAME:$API_PASSWORD -s -X DELETE "$SPIDER_URL/$TEST_BUCKET/$key?uploadId=$upload_id" \
...
wait_for_bucket_deletion() {
    ...
    while [[ $wait_time -lt $max_wait ]]; do
        ...
        check_response=$(curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -o /dev/null -I "$SPIDER_URL/$TEST_BUCKET" \
    -H "Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME")
...
cleanup_all_objects() {
    ...
    objects_response=$(curl -u $API_USERNAME:$API_PASSWORD -s -X GET "$SPIDER_URL/$TEST_BUCKET" \
  -H "Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME" 2>/dev/null)
    ...
    if [[ -n "$objects_response" ]] && [[ "$objects_response" =~ \<Key\> ]]; then
        ...
        echo "$objects_response" | grep -o '<Key>[^<]*</Key>' | sed 's/<[^>]*>//g' | while read -r key; do
            if [[ -n "$key" ]]; then
                ...
                curl -u $API_USERNAME:$API_PASSWORD -s -X DELETE "$SPIDER_URL/$TEST_BUCKET/$key" \
  -H "Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME" >/dev/null 2>&1
            fi
        done
    fi
...
cleanup() {
    ...
    if bucket_exists; then
        ...
        curl -u $API_USERNAME:$API_PASSWORD -s -X DELETE "$SPIDER_URL/$TEST_BUCKET?force=true" \
  -H "Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME" >/dev/null 2>&1
    else
...
main() {
    ...
    run_test "list_buckets" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "ListAllMyBucketsResult" \
        "List all buckets"
    
    run_test "create_bucket" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X PUT '$SPIDER_URL/$TEST_BUCKET' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "200" \
        "Create test bucket"
    ...
    run_test "get_bucket_info" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "ListBucketResult" \
        "Get bucket information"
    
    run_test "head_bucket" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -I '$SPIDER_URL/$TEST_BUCKET' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "200" \
        "Check bucket exists"
    
    run_test "get_bucket_location" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET?location' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "LocationConstraint" \
        "Get bucket location"
    ...
    DELETE_CREATE_RESPONSE=$(curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X PUT "$SPIDER_URL/$DELETE_BUCKET" \
    -H "Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME")
    ...
    if [[ "$DELETE_CREATE_CODE" == "200" || "$DELETE_CREATE_CODE" == "201" ]]; then
        ...
        run_test "delete_bucket" \
            "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X DELETE '$SPIDER_URL/$DELETE_BUCKET' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
            "204" \
            "Delete bucket"
    else
    ...
    run_test "upload_object_file" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X PUT '$SPIDER_URL/$TEST_BUCKET/$TEST_OBJECT' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' --data-binary '@$TEMP_DIR/$TEST_OBJECT'" \
    ...
    run_test "upload_object_form" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X POST '$SPIDER_URL/$TEST_BUCKET' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' -F 'key=form-upload.txt' -F 'file=@$TEMP_DIR/$TEST_OBJECT'" \
    ...
    run_test "download_object" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET/$TEST_OBJECT' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' -o '$TEMP_DIR/downloaded-file.txt' && cat '$TEMP_DIR/downloaded-file.txt'" \
    ...
    run_test "head_object" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -I '$SPIDER_URL/$TEST_BUCKET/$TEST_OBJECT' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "200" \
        "Get object info"
    
    run_test "delete_object" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X DELETE '$SPIDER_URL/$TEST_BUCKET/form-upload.txt' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "204" \
        "Delete single object"
    
    run_test "delete_multiple_objects" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X POST '$SPIDER_URL/$TEST_BUCKET?delete' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' -d '<Delete><Object><Key>$TEST_OBJECT</Key></Object></Delete>'" \
    ...
    curl -u $API_USERNAME:$API_PASSWORD -s -X PUT "$SPIDER_URL/$TEST_BUCKET/multipart-test.txt" \
    ...
    run_test "initiate_multipart" \
        "UPLOAD_ID=\$(curl -u $API_USERNAME:$API_PASSWORD -s -X POST '$SPIDER_URL/$TEST_BUCKET/multipart-large.txt?uploads' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' | grep -o '<UploadId>[^<]*</UploadId>' | sed 's/<[^>]*>//g'); echo \"UploadId: \$UPLOAD_ID\"" \
    ...
    -H "Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME" --data-binary "@$TEMP_DIR/large-file.txt" -I)
        ...
        run_test "list_parts" \
            "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET/multipart-large.txt?uploadId=$UPLOAD_ID&list-type=parts' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
            "ListPartsResult" \
            "List parts"
        
        run_test "abort_multipart" \
            "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X DELETE '$SPIDER_URL/$TEST_BUCKET/multipart-large.txt?uploadId=$UPLOAD_ID' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
            "204" \
    ...
    NEW_UPLOAD_ID=$(curl -u $API_USERNAME:$API_PASSWORD -s -X POST "$SPIDER_URL/$TEST_BUCKET/multipart-complete.txt?uploads" \
    -H "Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME" | grep -o '<UploadId>[^<]*</UploadId>' | sed 's/<[^>]*>//g')
    if [[ -n "$NEW_UPLOAD_ID" ]]; then
        # Upload part and get real ETag
        PART_UPLOAD_RESPONSE=$(curl -u $API_USERNAME:$API_PASSWORD -s -w '\n%{http_code}' -X PUT "$SPIDER_URL/$TEST_BUCKET/multipart-complete.txt?partNumber=1&uploadId=$NEW_UPLOAD_ID" \
    -H "Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME" --data-binary "@$TEMP_DIR/large-file.txt" -I)
        REAL_ETAG=$(echo "$PART_UPLOAD_RESPONSE" | grep -i "etag:" | cut -d':' -f2 | tr -d ' \r\n"' | tr -d '"')
        
        if [[ -n "$REAL_ETAG" ]]; then
            run_test "complete_multipart" \
                "curl -u $API_USERNAME:$API_PASSWORD -s -X POST '$SPIDER_URL/$TEST_BUCKET/multipart-complete.txt?uploadId=$NEW_UPLOAD_ID' \
  ...
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' -d '<CompleteMultipartUpload><Part><PartNumber>1</PartNumber><ETag>\"test-etag\"</ETag></Part></CompleteMultipartUpload>'" \
    ...
    run_test "list_multipart_uploads" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET?uploads' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
    ...
    run_test "get_bucket_versioning" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET?versioning' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "VersioningConfiguration" \
        "Get bucket versioning status"
    
    run_test "set_bucket_versioning" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X PUT '$SPIDER_URL/$TEST_BUCKET?versioning' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' -d '<VersioningConfiguration><Status>Enabled</Status></VersioningConfiguration>'" \
    ...
    run_test "list_object_versions" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET?versions' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "ListVersionsResult" \
        "List object versions"
    
    run_test "delete_versioned_object" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X DELETE '$SPIDER_URL/$TEST_BUCKET/multipart-test.txt' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "204" \
    ...
    run_test "set_bucket_cors" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X PUT '$SPIDER_URL/$TEST_BUCKET?cors' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' -d '<CORSConfiguration><CORSRule><AllowedOrigin>*</AllowedOrigin><AllowedMethod>GET</AllowedMethod><AllowedMethod>PUT</AllowedMethod><AllowedHeader>*</AllowedHeader></CORSRule></CORSConfiguration>'" \
    ...
    run_test "get_bucket_cors" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET?cors' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "CORSRule" \
        "Get bucket CORS configuration"
    
    run_test "test_cors_options" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X OPTIONS '$SPIDER_URL/$TEST_BUCKET' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' -H 'Origin: http://example.com' -H 'Access-Control-Request-Method: GET'" \
    ...
    run_test "delete_bucket_cors" \
        "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X DELETE '$SPIDER_URL/$TEST_BUCKET?cors' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
        "204" \
    ...
    curl -u $API_USERNAME:$API_PASSWORD -s -X PUT "$SPIDER_URL/$TEST_BUCKET/presigned-test.txt" \
    ...
    run_test "generate_presigned_download" \
        "PRESIGNED_DOWNLOAD_URL=\$(curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET/presigned-test.txt?presigned&duration=3600' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' | grep -o '<PresignedURL>[^<]*</PresignedURL>' | sed 's/<[^>]*>//g'); echo \"Generated URL: \${PRESIGNED_DOWNLOAD_URL:0:50}...\"" \
    ...
    run_test "generate_presigned_upload" \
        "PRESIGNED_UPLOAD_URL=\$(curl -u $API_USERNAME:$API_PASSWORD -s -X GET '$SPIDER_URL/$TEST_BUCKET/presigned-upload-test.txt?presigned&upload&duration=3600' \
  -H 'Authorization: AWS4-HMAC-SHA256 Credential=$CONNECTION_NAME' | grep -o '<PresignedURL>[^<]*</PresignedURL>' | sed 's/<[^>]*>//g'); echo \"Generated URL: \${PRESIGNED_UPLOAD_URL:0:50}...\"" \
    ...
    if bucket_exists; then
        ...
        run_test "force_empty_bucket" \
            "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X DELETE '$SPIDER_URL/$TEST_BUCKET?empty=true' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
            "204" \
            "Force empty bucket"
    else
    ...
    if bucket_exists; then
        ...
        run_test "force_delete_bucket" \
            "curl -u $API_USERNAME:$API_PASSWORD -s -w '%{http_code}' -X DELETE '$SPIDER_URL/$TEST_BUCKET?force=true' \
    -H \"Authorization: AWS4-HMAC-SHA256 Credential=\$CONNECTION_NAME\"" \
            "204" \
            "Force delete bucket"
    else
...
check_server() {
    if ! curl -u $API_USERNAME:$API_PASSWORD -s "$SPIDER_URL" >/dev/null 2>&1; then
