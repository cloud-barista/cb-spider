...
#### install nginx
ssh -i $1-keypair-01.pem cb-user@$public_ip "sudo apt-get update"
ssh -i $1-keypair-01.pem cb-user@$public_ip "sudo apt-get install -y nginx"


### setup index.html with public ip

ssh -i $1-keypair-01.pem cb-user@$public_ip "sudo sed -i 's/nginx\!/'"${public_ip}"'/g' /var/www/html/index.nginx-debian.html"
