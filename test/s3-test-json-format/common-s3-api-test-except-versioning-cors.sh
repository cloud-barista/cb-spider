...
# Test 24 S3 APIs with JSON input/output format
...
main() {
    ...
    # ========================================
    # 5. CORS MANAGEMENT TESTS (4/4) - SKIPPED
    # ========================================
    ...
    run_test "generate_presigned_download" \
        "PRESIGNED_DOWNLOAD_URL=\$(curl -u $SPIDER_USERNAME:$SPIDER_PASSWORD -s -H 'Accept: application/json' -X GET '$SPIDER_URL/presigned/download/$TEST_BUCKET/presigned-test.txt?ConnectionName=$CONNECTION_NAME&expires=3600' | jq -r '.PresignedURL // empty' 2>/dev/null); echo \"Generated URL: \${PRESIGNED_DOWNLOAD_URL:0:50}...\"" \
        "Generated URL:" \
        "Generate presigned download URL (JSON response)"
    
    # Extract presigned download URL for actual test (JSON format)
    PRESIGNED_DOWNLOAD_URL=$(curl -u $SPIDER_USERNAME:$SPIDER_PASSWORD -s -H 'Accept: application/json' -X GET "$SPIDER_URL/presigned/download/$TEST_BUCKET/presigned-test.txt?ConnectionName=$CONNECTION_NAME&expires=3600" | jq -r '.PresignedURL // empty' 2>/dev/null)
    ...
    run_test "generate_presigned_upload" \
        "PRESIGNED_UPLOAD_URL=\$(curl -u $SPIDER_USERNAME:$SPIDER_PASSWORD -s -H 'Accept: application/json' -X GET '$SPIDER_URL/presigned/upload/$TEST_BUCKET/presigned-upload-test.txt?ConnectionName=$CONNECTION_NAME&expires=3600' | jq -r '.PresignedURL // empty' 2>/dev/null); echo \"Generated URL: \${PRESIGNED_UPLOAD_URL:0:50}...\"" \
        "Generated URL:" \
        "Generate presigned upload URL (JSON response)"
    
    # Extract presigned upload URL for actual test (JSON format)
    PRESIGNED_UPLOAD_URL=$(curl -u $SPIDER_USERNAME:$SPIDER_PASSWORD -s -H 'Accept: application/json' -X GET "$SPIDER_URL/presigned/upload/$TEST_BUCKET/presigned-upload-test.txt?ConnectionName=$CONNECTION_NAME&expires=3600" | jq -r '.PresignedURL // empty' 2>/dev/null)
    ...
    if [[ -n "$PRESIGNED_UPLOAD_URL" ]]; then
        ...
        run_test "test_presigned_upload" \
            "echo 'Presigned upload test content' > '$TEMP_DIR/presigned-upload.txt' && curl -s ${_extra_hdr} -H 'Accept: application/json' -w '%{http_code}' -X PUT '$PRESIGNED_UPLOAD_URL' --data-binary '@$TEMP_DIR/presigned-upload.txt'" \
        "20[01]" \
