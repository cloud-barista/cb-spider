...
$CLIPATH/spctl  keypair get --cname "${CONN_CONFIG}" -n "${KEYPAIR_NAME}" 2> /dev/null
echo "============== after get KeyPair: '${KEYPAIR_NAME}'"

echo -e "\n\n"

echo "============== before get SecurityGroup: '${SG_NAME}'"
$CLIPATH/spctl  security get --cname "${CONN_CONFIG}" -n "${SG_NAME}" 2> /dev/null
echo "============== after get SecurityGroup: '${SG_NAME}'"

echo -e "\n\n"

echo "============== before get VPC/Subnet: '${VPC_NAME}'"
$CLIPATH/spctl  vpc get --cname "${CONN_CONFIG}" -n "${VPC_NAME}" 2> /dev/null
