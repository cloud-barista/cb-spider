...
curl -u $SPIDER_USERNAME:$SPIDER_PASSWORD -X DELETE http://$RESTSERVER:1024/spider/credential/azure-credential01 \
    -H 'Content-Type: application/json'
...
curl -u $SPIDER_USERNAME:$SPIDER_PASSWORD -X DELETE http://$RESTSERVER:1024/spider/driver/azure-driver01 \
    -H 'Content-Type: application/json'
