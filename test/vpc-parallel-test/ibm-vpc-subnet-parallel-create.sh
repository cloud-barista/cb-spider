...
usage() {
    ...
    echo "  vpc_count: number of VPC/Subnet pairs to create in parallel (1-999)"
}
...
LOG_DIR="${PWD}/ibm-vpc-parallel-logs-${NOW_UTC}"
mkdir -p "$LOG_DIR"
...
echo "[INFO] Start parallel create test"
echo "[INFO] API_URL         : $API_URL"
echo "[INFO] ConnectionName  : $CONNECTION_NAME"
echo "[INFO] Count           : $VPC_COUNT"
echo "[INFO] VPC CIDR        : $VPC_CIDR (AdminWeb IBM default)"
echo "[INFO] Subnet CIDR     : $SUBNET_CIDR (AdminWeb default)"
echo "[INFO] Subnet Zone     : $SUBNET_ZONE (ibm-us-south-1-config default zone)"
...
while [ "$i" -le "$VPC_COUNT" ]; do
    ...
    SUBNET_NAME="${NAME_PREFIX}-${INDEX}-subnet-01"

    REQ_FILE="${LOG_DIR}/${VPC_NAME}.request.json"
