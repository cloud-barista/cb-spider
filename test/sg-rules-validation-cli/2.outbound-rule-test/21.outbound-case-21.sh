...
echo "============== before RemoveRules: '${SG_NAME}' --- outbound:TCP/1000/1000/${C_IP}/32"
...
echo "============== after RemoveRules: '${SG_NAME}' --- outbound:TCP/1000/1000/${C_IP}/32"

# Get SG info two times to give a slice time to csp
curl -u $SPIDER_USERNAME:$SPIDER_PASSWORD -sX GET http://localhost:1024/spider/securitygroup/SG-Rules-Test-SG01 -H 'Content-Type: application/json' -d '{ "ConnectionName": "'${CONN_CONFIG}'" }' 1> /dev/null 2>/dev/null
curl -u $SPIDER_USERNAME:$SPIDER_PASSWORD -sX GET http://localhost:1024/spider/securitygroup/SG-Rules-Test-SG01 -H 'Content-Type: application/json' -d '{ "ConnectionName": "'${CONN_CONFIG}'" }' 1> /dev/null 2>/dev/null
...
./io-traffic-test.sh $1    pass    fail      skip     fail         pass     fail     pass    pass
