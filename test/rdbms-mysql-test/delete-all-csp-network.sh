...
# CB-Spider Network Prerequisite Cleanup Script for All CSPs
# Deletes the VPC/Subnet (and Security Group, for AWS) created by
# run-all-csp-network-prepare.sh, on all 9 CSPs in parallel.
...
# ── Result table ──────────────────────────────────────────────────────────────
echo "============================================================"
echo "     NETWORK PREREQUISITE CLEANUP SUMMARY - ALL CSPs"
echo "============================================================"
echo ""
...
for csp in ${CSP_ORDER}; do
    ...
    [[ "${r_vpc}" == "NOT_FOUND_OR_ERROR" || "${r_vpc}" == "NO_RESULT" ]] && fail_count=$((fail_count + 1))
    [[ "${r_sg}" == "NOT_FOUND_OR_ERROR" ]] && fail_count=$((fail_count + 1))
done
