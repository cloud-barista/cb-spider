...
# CB-Spider RDBMS Tag Management Test Runner
# Runs AddTag/ListTag/GetTag/RemoveTag tests for CSPs where RDBMSMetaInfo.SupportsTag=true.
# Supported CSPs: AWS, AZURE, GCP, ALIBABA, TENCENT, IBM
...
csp_script() {
    case "$1" in
        ...
        AZURE)   echo "azure-rdbms-tag-test.sh"   ;;
        GCP)     echo "gcp-rdbms-tag-test.sh"     ;;
        ALIBABA) echo "alibaba-rdbms-tag-test.sh" ;;
        TENCENT) echo "tencent-rdbms-tag-test.sh" ;;
        IBM)     echo "ibm-rdbms-tag-test.sh"     ;;
    esac
}
...
print_header() {
    echo ""
    printf '%115s\n' '' | tr ' ' '='
    echo "                   RDBMS TAG MANAGEMENT TEST SUMMARY (SupportsTag=true CSPs)"
    printf '%115s\n' '' | tr ' ' '='
    echo ""
...
echo ""
echo "################################################################################"
echo "#      CB-Spider RDBMS Tag Management Test - SupportsTag=true CSPs Only       #"
echo "#                  AWS / AZURE / GCP / ALIBABA / TENCENT / IBM                #"
echo "################################################################################"
echo ""
...
# Only CSPs where RDBMSMetaInfo.SupportsTag=true
CSP_ORDER="AWS AZURE GCP ALIBABA TENCENT IBM"
